...
public class Battle {
	...
	private long doFFABattle(int itier) {
		...
		while (getAliveTeamCount() > 1) {
			...
			if (!defend.getDinosaur().isAlive()) {
				...
				if (attack.getPlayer().getIdLong() > CustomPlayer.getUpperLimit()) {
					...
					JDBC.addItem(attack.getPlayer().getIdLong(), Stats.of(Stats.DINOSAURS_DEFEATED));
				}
				...
				if (defend.getPlayer().getIdLong() == CustomPlayer.Wild.getIdLong()) {
					...
					Action.addDinosaurDelayed(attack.getPlayer().getIdLong(), time + 1000, defend.getDinosaur().getId());
					if (defend.getDinosaur().hasRune()) Action.addRuneDelayed(attack.getPlayer().getIdLong(), time + 1000, defend.getDinosaur().getRune().getId());
					if (defend.getDinosaur().hasItem()) Action.addItemDelayed(attack.getPlayer().getIdLong(), time + 1000, defend.getDinosaur().getItem().getIdDmg(), 1);
				}
				...
				if (attack.getPlayer().getIdLong() > CustomPlayer.getUpperLimit()) {
					...
					Action.addXpDelayed(attack.getPlayer().getIdLong(), time, attack.getDinosaur().getId(), Math.round(mult * DinoMath.getXpDropped(defend.getDinosaur().getLevel())));
				}
		...
		if (winner.getPlayer().getIdLong() == CustomPlayer.Wild.getIdLong()) {
			...
			// Stat - Battles Won
			Action.addItemDelayed(winner.getPlayer().getIdLong(), time, Stats.of(Stats.BATTLES_WON), 1);
			...
			case 0: {
				...
				Pair<Integer, Long> money = new Pair<Integer, Long>(100, 0L);
		...
		if (atkeff == BattleAttack.Rune) {
			if (attack.getDinosaur().hasRune()) {
				...
				case DealDamage:
					...
					Action.addItemDelayed(attack.getPlayer().getIdLong(), time, Stats.of(Stats.DAMAGE_DEALT), Math.min(damage, defend.getDinosaur().getCurrentHealth()));
					Action.addItemDelayed(defend.getPlayer().getIdLong(), time, Stats.of(Stats.DAMAGE_RECEIVED), Math.min(damage, defend.getDinosaur().getCurrentHealth()));
